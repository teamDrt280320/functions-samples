...
const smsalertApiKey='60cda38ac4c24';
...
//     var mobileno=user.phoneNumber;

//     axios.post(
//       `https://www.smsalert.co.in/api/push.json?apikey=${smsalertApiKey}&sender=DCTBZR&mobileno=${mobileno}&text=Hello ${userName},Thank you for registering with Discount Bazaar.Please check your email for your account details.Log in to the app and shop wide range of products across categories.`,
...
//     console.log(`Successfully fetched phoneNumber: ${mobileno}`);
//   })
//   .catch((error) => {
...
exports.sendOrderPlaced=functions.firestore.document('users/{uid}/orders/{orderId}').onCreate(async (order,context)  =>{
  ...
  //     });
      ...
      const smsPromise=  admin
      .auth()
      .getUser(uid)
      .then((user) => {
    
        var mobileno=user.phoneNumber;
        var orderId='#'.order.data()['time'];
        
    
        axios.post(
          `https://www.smsalert.co.in/api/push.json?apikey=${smsalertApiKey}&sender=DCTBZR&mobileno=${mobileno}&text=Hello ${name}, Thank you for your order ${orderId} with Discount Bazaar. We will notify you when it gets dispatched along with the delivery details.`
        ).then(function (response) {
          console.log(response);
        })
        .catch(function (error) {
